const app = Vue.createApp({
  ...
  computed: {
    filteredData() {
      const baseData = this.dateFilterActive
        ? this.dateFilteredItems
        : this.items;
      return baseData.filter((item) => {
        // 新增「未請購、已請購」的切換條件
        if (this.filterPurchaseStatus === "ORDERED" && item["開單狀態"] !== "V")
          return false;
        if (
        ...
        const matchReceivingResult =
          this.checkedReceivingResults.length === 0 ||
          this.checkedReceivingResults.includes(
            (item["驗收狀態"] ?? "").trim()
          );
        const matchPerson =
          this.checkedPeople.length === 0 ||
          this.checkedPeople.includes(item["需求者"]);
        const matchState =
          this.checkedStates.length === 0 ||
          this.checkedStates.includes(item["開單狀態"]);
        const matchWBS =
          this.checkedWBS.length === 0 || this.checkedWBS.includes(item["WBS"]);
        const matchOrder =
          this.checkedOrders.length === 0 ||
          this.checkedOrders.includes(String(item["請購順序"]).trim());
        const formattedNeedDate =
          String(item["需求日"]).length === 8
            ? `${String(item["需求日"]).slice(0, 4)}/${String(
                item["需求日"]
              ).slice(4, 6)}/${String(item["需求日"]).slice(6, 8)}`
            : item["需求日"];
        const matchNeedDate =
          this.checkedNeedDates.length === 0 ||
          this.checkedNeedDates.includes(formattedNeedDate);
        const issuedMonth = String(item["已開單日期"]);
        const formattedIssuedMonth =
          issuedMonth.length === 8 ? issuedMonth.slice(0, 6) : issuedMonth;
        const matchIssuedMonth =
          this.checkedIssuedMonths.length === 0 ||
          this.checkedIssuedMonths.includes(formattedIssuedMonth);
        const matchEPR =
          this.checkedEPRs.length === 0 ||
          this.checkedEPRs.includes(item["ePR No."]);
        const poValues = String(item["PO No."] || "")
          .split(/<br\s*\/?>|\r?\n/)
          .map((v) => v.trim())
          .filter(Boolean);
        const matchPONo =
          this.checkedPONos.length === 0 ||
          poValues.some((v) => this.checkedPONos.includes(v));
        const matchItem =
          this.checkedItems.length === 0 ||
          this.checkedItems.includes(item["請購項目"]);
        const matchReason =
          this.checkedReasons.length === 0 ||
          this.checkedReasons.includes(item["需求原因"]);
        const matchAmount =
          this.checkedAmounts.length === 0 ||
          this.checkedAmounts.includes(String(item["總金額"]).trim());
        const matchStage =
          this.checkedStages.length === 0 ||
          this.checkedStages.includes(item["簽核中關卡"]);
        const matchStatus =
          this.checkedStatuses.length === 0 ||
          this.checkedStatuses.includes(item["Status"]);
        const matchRemark =
          this.checkedRemarks.length === 0 ||
          this.checkedRemarks.includes(item["備註"]);

        return (
          matchReceivingResult &&
          matchPerson &&
          matchState &&
          matchWBS &&
          matchOrder &&
      ...
      return Array.from(
        new Set(
          baseData
            .filter((i) => {
              if (
                this.filterPurchaseStatus === "ORDERED" &&
                i["開單狀態"] !== "V"
              )
                return false;
              if (
              ...
              const matchState =
                this.checkedStates.length === 0 ||
                this.checkedStates.includes(i["開單狀態"]);
              const matchReceivingResult =
                this.checkedReceivingResults.length === 0 ||
                this.checkedReceivingResults.includes(
                  (i["驗收狀態"] ?? "").trim()
              ...
              const matchWBS =
                this.checkedWBS.length === 0 ||
                this.checkedWBS.includes(i["WBS"]);
              const matchOrder =
                this.checkedOrders.length === 0 ||
                this.checkedOrders.includes(String(i["請購順序"]).trim());
              const formattedNeedDate =
                String(i["需求日"]).length === 8
                  ? `${String(i["需求日"]).slice(0, 4)}/${String(
                      i["需求日"]
                    ).slice(4, 6)}/${String(i["需求日"]).slice(6, 8)}`
                  : i["需求日"];
              const matchNeedDate =
                this.checkedNeedDates.length === 0 ||
                this.checkedNeedDates.includes(formattedNeedDate);
              const formattedIssuedMonth =
                String(i["已開單日期"]).length === 8
              ...
              const matchEPR =
                this.checkedEPRs.length === 0 ||
                this.checkedEPRs.includes(i["ePR No."]);
              const matchPONo =
                this.checkedPONos.length === 0 ||
                this.checkedPONos.includes(item["PO No."]);
              const matchItem =
                this.checkedItems.length === 0 ||
                this.checkedItems.includes(i["請購項目"]);
              const matchReason =
                this.checkedReasons.length === 0 ||
                this.checkedReasons.includes(i["需求原因"]);
              const matchAmount =
                this.checkedAmounts.length === 0 ||
                this.checkedAmounts.includes(String(i["總金額"]).trim());
              const matchStage =
                this.checkedStages.length === 0 ||
                this.checkedStages.includes(i["簽核中關卡"]);
              const matchStatus =
                this.checkedStatuses.length === 0 ||
                this.checkedStatuses.includes(i["Status"]);
              const matchRemark =
                this.checkedRemarks.length === 0 ||
                this.checkedRemarks.includes(i["備註"]);
        ...
        new Set(
          baseData
            .filter((i) => {
              ...
              const matchPerson =
                this.checkedPeople.length === 0 ||
                this.checkedPeople.includes(i["需求者"]);
              const matchWBS =
                this.checkedWBS.length === 0 ||
                this.checkedWBS.includes(i["WBS"]);
              const matchOrder =
              ...
              const matchPONo =
                this.checkedPONos.length === 0 ||
                this.checkedPONos.includes(i["PO No."]);
              const matchRemark =
                this.checkedRemarks.length === 0 ||
                this.checkedRemarks.includes(i["備註"]);
        ...
        new Set(
          baseData
            .filter((i) => {
              ...
              const formattedIssuedMonth =
                issuedMonth.length === 8 ? issuedMonth.slice(0, 6) : "";
              const matchIssuedMonth =
                this.checkedIssuedMonths.length === 0 ||
                this.checkedIssuedMonths.includes(formattedIssuedMonth);
              const matchText =
                this.ePRsSearchText === "" ||
                i["ePR No."]?.includes(this.ePRsSearchText.trim());
              const matchPONo =
                this.checkedPONos.length === 0 ||
                this.checkedPONos.includes(i["PO No."]);
              const matchItem =
    ...
    uniquePONos() {
      ...
      baseData
        .filter((i) => {
          if (this.filterPurchaseStatus === "ORDERED" && i["開單狀態"] !== "V")
            return false;
          if (
            this.filterPurchaseStatus === "UNORDERED" &&
            i["開單狀態"] === "V"
          ...
          const matchReceivingResult =
            this.checkedReceivingResults.length === 0 ||
            this.checkedReceivingResults.includes((i["驗收狀態"] ?? "").trim());
          const matchPerson =
            this.checkedPeople.length === 0 ||
            this.checkedPeople.includes(i["需求者"]);
          const matchState =
            ...
            this.checkedStates.includes(i["開單狀態"]);
          const matchWBS =
            this.checkedWBS.length === 0 || this.checkedWBS.includes(i["WBS"]);
          const matchOrder =
            this.checkedOrders.length === 0 ||
            this.checkedOrders.includes(String(i["請購順序"]).trim());
          const formattedNeedDate =
            String(i["需求日"]).length === 8
              ? `${String(i["需求日"]).slice(0, 4)}/${String(i["需求日"]).slice(
                  4,
                  6
                )}/${String(i["需求日"]).slice(6, 8)}`
              : i["需求日"];
          const matchNeedDate =
            this.checkedNeedDates.length === 0 ||
            this.checkedNeedDates.includes(formattedNeedDate);
        ...
        new Set(
          baseData
            .filter((i) => {
              ...
              const matchPONo =
                this.checkedPONos.length === 0 ||
                this.checkedPONos.includes(it["PO No."]);
              const matchText =
                this.itemSearchText === "" ||
                item["請購項目"]?.includes(this.itemSearchText.trim());
              const matchReason =
                this.checkedReasons.length === 0 ||
                this.checkedReasons.includes(i["需求原因"]);
              const matchAmount =
        ...
        new Set(
          baseData
            .filter((i) => {
              ...
              const matchText =
                this.itemSearchText === "" ||
                i["請購項目"]?.includes(this.itemSearchText.trim());
              const matchAmount =
                this.checkedAmounts.length === 0 ||
                this.checkedAmounts.includes(String(i["總金額"]).trim());
              const matchStage =
          ...
          baseData
            .filter((i) => {
              ...
              const matchText =
                ...
                i["備註"]?.includes(this.remarkSearchText.trim());
    ...
    filteredUnorderedCount() {
      return this.filteredData.filter((item) => item["開單狀態"] !== "V")
        .length;
    },
    ...
    allUnorderedCountMoney() {
      return this.items
        .reduce((sum, item) => {
          if (item["開單狀態"] !== "V") {
            const amount =
              parseFloat(String(item["總金額"]).replace(/,/g, "").trim()) || 0;
            return sum + amount;
          }
          return sum;
        }, 0)
  ...
  methods: {
    ...
    async saveFiltersToJSON() {
      ...
      try {
        const response = await fetch(
          "http://127.0.0.1:5000/api/save-filters-json",
          {
            method: "POST",
    ...
    async loadFiltersFromJSON() {
      ...
      try {
        const response = await fetch(
          `http://127.0.0.1:5000/api/get-filters-json/${this.username}`
        );
        ...
        if (response.ok) {
          ...
          if (filters) {
            ...
            (this.checkedReceivingResults =
              filters.checkedReceivingResults || []),
    ...
    handleClickOutside(event) {
      ...
      const isReceivingResult =
        this.$refs.ReceivingResultDropdownWrapper?.contains(event.target);
      ...
      const isNeedPerson = this.$refs.NeedPersonDropdownWrapper?.contains(
        event.target
      );
      const isItem = this.$refs.NeedItemDropdownWrapper?.contains(event.target);
      const isReason = this.$refs.NeedReasonDropdownWrapper?.contains(
        event.target
      );
      const isAmount = this.$refs.TotalMoneyDropdownWrapper?.contains(
        event.target
      );
      const isNeedDate = this.$refs.NeedDateDropdownWrapper?.contains(
        event.target
      );
      const isIssuedDate = this.$refs.AleadyDateDropdownWrapper?.contains(
        event.target
      );
      ...
      const isRemark = this.$refs.RemarksDropdownWrapper?.contains(
        event.target
      );
    ...
    async editItem(index, item) {
      ...
      const res = await fetch(
        `http://127.0.0.1:5000/api/get_detail/${item.Id}`
      );
      ...
      if (
        this.editItemData["ePR No."] &&
        this.editItemData["ePR No."].trim() !== ""
      ) {
      ...
      if (
        this.editItemData["需求者"] &&
        this.editItemData["需求者"].trim() !== ""
      ) {
        this.editTableRows = this.editTableRows.map((row) => ({
          ...row,
    ...
    async uploadMoney() {
      ...
      try {
        const response = await axios.post(
          "http://127.0.0.1:5000/api/uploadMoney",
          payload
        );
    ...
    async openNewItemModal() {
      ...
      try {
        const res = await axios.post(
          "http://127.0.0.1:5000/api/get-username-info",
          {
    ...
    parseEPR(val, target) {
      ...
      target[
        "進度追蹤超連結"
      ] = `https://khwfap.kh.asegroup.com/ePR/PRQuery/QueryPR?id=${val}`;
      target["已開單日期"] = `20${year}/${String(month).padStart(
        2,
        "0"
    ...
    formatDateInput(val) {
      ...
      if (/^\d{8}$/.test(strVal)) {
        return `${strVal.slice(0, 4)}-${strVal.slice(4, 6)}-${strVal.slice(
          6,
          8
        )}`;
      }
    ...
    async checkEditPermission(id) {
      ...
      try {
        const res = await axios.post(
          "http://127.0.0.1:5000/api/check-edit-permission",
          {
            id,
            currentUser,
          }
    ...
    async addNewEditRow() {
      ...
      const nextItemNumber = String(
        (this.editTableRows.length + 1) * 10
      ).padStart(4, "0");
    ...
    handlePaste(event, targetObj, key) {
      ...
      // 貼上文字
      const newValue =
        originalValue.slice(0, start) + pastedData + originalValue.slice(end);
      // 重新賦予文字
    ...
    async resetAllFilters() {
      ...
      this.checkedPeople = [];
      this.checkedStates = [];
      (this.checkedReceivingResults = []), (this.checkedWBS = []);
      this.checkedOrders = [];
      ...
      (this.ePRsSearchText = ""), (this.sortField = "");
      this.sortOrder = "asc";
      this.filterStartDate = "";
      this.filterEndDate = "";
      this.dateFilterActive = false;
      ...
      try {
        await fetch(
          `http://127.0.0.1:5000/api/clear-filters-json/${this.username}`,
          {
            method: "DELETE",
          }
        );
    ...
    async sendItemEmail() {
      ...
      if (
        !this.editItemData["簽核中關卡"] ||
        String(this.editItemData["簽核中關卡"]).trim() === ""
      ) {
