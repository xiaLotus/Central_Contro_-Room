...
def save_csv():
    ...
    mismatch_po_set = set()  # 🆕 記錄哪些 PO 有分批問題
    ...
    for file_info in saved_files:
        ...
        for item, xls_group in xls_item_groups:
            ...
            if csv_count > 0 and xls_count > csv_count:
                mismatch_po_set.add(po_no)  # 🆕 記錄這個 PO 有問題
    ...
    # 🔴 第二輪:處理沒有分批問題的 PO (正常比對)
    for file_info in saved_files:
        po_no = file_info["po_no"]
        
        # 🆕 如果這個 PO 有分批問題,跳過正常比對
    ...
    if quantity_mismatch_items:
        ...
        return jsonify({
            ...
            "groups": all_group_results,  # 🆕 同時返回正常的 PO 比對結果
            "has_normal_items": len(all_group_results) > 0  # 🆕 標記是否有正常項目
        })
